...
const DialogOverlay = React.forwardRef<DialogOverlayElement, DialogOverlayProps>((props, ref) => {
  ...
  return (
    <DialogPrimitive.Overlay
      ref={ref}
      className={cn(
        'bg-background/80 fixed inset-0 z-20 grid place-items-center overflow-y-auto',
    ...
    <DialogPortal>
      <DialogOverlay>
        <DialogPrimitive.Content
          ref={ref}
          className={cn(
            'bg-secondary fixed left-[50%] top-[50%] z-50 grid w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-6 border p-8 shadow-lg duration-200 sm:rounded-lg',
            'data-[state=open]:animate-content-show data-[state=closed]:animate-content-hide transition-all ease-out',
            className,
          )}
          {...contentProps}
        >
